...
import com.bluemobi.util.SocketOperate;
...
public class WebAlarmController extends AbstractAPIController {
	...
	try {
	    ...
	    Map<String, Object> reqMap = new HashMap<String, Object>();
	    // 查询报警-－固定取5条最新的报警纪录
	    page = alarmManageService.page(reqMap, 1, 5);
     ...
     * @return
     */
    @RequestMapping(value = "getHumForAlarm")
    @ResponseBody
    public ResultTO getHumForAlarm(HttpServletRequest request) {
    	String hum = null;
    	if(SocketOperate.temperatureList.size()>0){
    		hum = SocketOperate.temperatureList.get(SocketOperate.temperatureList.size()-1);
    	}
    	
		return ResultTO.newSuccessResultTO("请求成功", hum);
